...
public class ZoneAdaper extends RecyclerView.Adapter<ZoneAdaper.ViewHolder> {
    ...
    public ZoneAdaper(List<AVObject> itemList,Context context) {
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        ...
        Picasso.with(mContext).load(taskItemList.get(position).getAVFile("image") == null ? "www" : taskItemList.get(position).getAVFile("image").getUrl()).transform(new RoundedTransformation(9, 0)).into(holder.mPicture);
        Picasso.with(mContext).load(taskItemList.get(position).getAVUser("owner").getAVFile(AVATAR).getUrl())
                .placeholder(R.drawable.lcim_default_avatar_icon).into(holder.userimg);//头像
        holder.mItem.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(mContext, Task_full_infomation.class);//进入详情
